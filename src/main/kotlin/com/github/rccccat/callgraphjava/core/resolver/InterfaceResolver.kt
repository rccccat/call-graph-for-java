...
import com.github.rccccat.callgraphjava.util.buildClassKey
import com.github.rccccat.callgraphjava.util.resolveValidPointers
import com.github.rccccat.callgraphjava.util.toSmartPointer
import com.github.rccccat.callgraphjava.util.toSmartPointerList
...
class InterfaceResolver(
  ...
  private val inheritingClassesCache =
      cacheManager.createCachedValue {
        ConcurrentHashMap<String, List<SmartPsiElementPointer<PsiClass>>>()
      }
  ...
  private fun findInheritingClasses(baseClass: PsiClass): List<PsiClass> {
    val key = buildClassKey(baseClass)
  ...
  private fun findImplementationMethod(
    ...
    val baseOwnerKey = baseMethod.containingClass?.let { buildClassKey(it) } ?: "UnknownClass"
    val signature = baseMethod.getSignature(PsiSubstitutor.EMPTY).toString()
    val implClassKey = buildClassKey(implClass)
    val key = "$implClassKey#$baseOwnerKey#$signature"
