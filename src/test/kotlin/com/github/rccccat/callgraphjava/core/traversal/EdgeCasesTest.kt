...
class EdgeCasesTest : BasePlatformTestCase() {
  ...
  fun testSuperMethodCall() {
    ...
    val hasEdgeToAfterProcess = hasEdgeById(graph, childProcessNode?.id, afterProcessNode?.id)
    ...
    if (parentProcessNode != null) {
      val hasEdgeToParentProcess = hasEdgeById(graph, childProcessNode?.id, parentProcessNode.id)
      ...
      if (doWorkNode != null) {
        val hasEdgeToDoWork = hasEdgeById(graph, parentProcessNode.id, doWorkNode.id)
