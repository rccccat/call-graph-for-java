...
class CallGraphServiceJavaTest : BasePlatformTestCase() {
    ...
    fun testExcludePackagePatternSkipsCall() {
        updateSettings { excludePackagePatterns = mutableListOf("demo\\.external\\..*") }
        assertEquals(listOf("demo\\.external\\..*"), CallGraphAppSettings.getInstance().excludePackagePatterns)
        val file =
            myFixture.addFileToProject(
        ...
        val runCall =
            PsiTreeUtil
                .findChildrenOfType(handleMethod, com.intellij.psi.PsiMethodCallExpression::class.java)
        ...
        val qualifiedName = resolvedRun!!.containingClass!!.qualifiedName!!
        assertTrue(Regex(excludePattern).matches(qualifiedName))
